...
npm run verify         # the gate: 8 checks, everything, ~4 s — run it often
...
`llms-full.txt`, `index.html`'s HOME-FAST-PATH / HOME-PRERENDER blocks, every
published tool count, and `local-ai-knowledge.json`.

**Change the site brain.** Editing `README.md`, `agents.html`, `ai.html`,
`CONSTRAINTS.md`, `ARCHITECTURE.md`, `STRATEGY.md`, `INCOME.md`,
`learning/approved.json` or `scripts/build-site-brain.py` changes its inputs:
run `npm run build:brain` and commit the regenerated
`local-ai-knowledge.json`. `verify.sh` fails if you forget.
...
`CONSTRAINTS.md` (the hard lines, in full) · `ARCHITECTURE.md` (how the site
is built) · `docs/TRUST.md` (YMYL) · `docs/INSTRUMENTATION.md` (analytics) ·
