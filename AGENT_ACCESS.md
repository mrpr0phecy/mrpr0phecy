...
# Music work (omit the 602 cards too):
...
1. Write `cards/<tool-name>.html` — an **HTML fragment** (no `<!doctype>`,
   ...
   prefix (`xyz-…`) because all 602 cards share one DOM. Wrap all JS in an
...
- Cards: **602**, all indexed (no orphans), all with title+description.
