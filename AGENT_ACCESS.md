...
# Music work (omit the 493 cards too):
...
|---|---|---|
| **A — The Most Useful Site In The World** | `index.html` | **493** self-contained offline browser tools, indexed by `cards/cards.json` |
...
1. Write `cards/<tool-name>.html` — an **HTML fragment** (no `<!doctype>`,
   ...
   prefix (`xyz-…`) because all 493 cards share one DOM. Wrap all JS in an
...
3. Bump the count in `index.html` (`Search 493` → `Search 494`).
...
- Cards: **493**, all indexed (no orphans), all with title+description.
