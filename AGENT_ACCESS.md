...
# Music work (omit the 494 cards too):
...
|---|---|---|
| **A — The Most Useful Site In The World** | `index.html` | **494** self-contained offline browser tools, indexed by `cards/cards.json` |
...
1. Write `cards/<tool-name>.html` — an **HTML fragment** (no `<!doctype>`,
   ...
   prefix (`xyz-…`) because all 494 cards share one DOM. Wrap all JS in an
...
3. Bump the count in `index.html` (`Search 494` → `Search 495`).
...
- Cards: **494**, all indexed (no orphans), all with title+description.
