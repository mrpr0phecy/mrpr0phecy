#!/usr/bin/env python3
"""
staffroom/scan.py — generate the staffroom BOARD from real git state.
...
Usage:
    python3 staffroom/scan.py             # print the report
    python3 staffroom/scan.py --write     # (re)write staffroom/BOARD.md
    python3 staffroom/scan.py --mine      # only collisions affecting HEAD
    python3 staffroom/scan.py --fetch     # refresh branch data from origin first

Read-only with respect to the repo: it never checks anything out, never
alters your working tree, and only writes staffroom/BOARD.md with --write.
It uses lightweight tags (staffroom-peek/*) to pin fetched commits, because
...
def read_notes():
    """branch_id -> owner/role, from staffroom/notes/<id>.md front matter."""
    owners = {}
...
def render(base, rows, contested):
    ...
    L = []
    L.append("# 📋 BOARD — who is working on what")
    L.append("")
    L.append("<!-- AUTO-GENERATED by staffroom/scan.py. Do not hand-edit: -->")
    ...
    L.append("")
    L.append("This is measured from real branch diffs, not from what agents say")
    L.append("they are doing. If a note in `notes/` disagrees with this table,")
    L.append("believe this table.")
    L.append("")
    ...
    L.append("style preference — see `DECISIONS.md` D-002.")
    L.append("")
    ...
    L.append("---")
    L.append("")
    L.append("Regenerate with `python3 staffroom/scan.py --write`.")
    return "\n".join(L) + "\n"
...
def main():
    ...
    ap.add_argument("--write", action="store_true", help="write staffroom/BOARD.md")
