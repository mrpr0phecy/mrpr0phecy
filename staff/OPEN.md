...
| P3-T8 | The card scanners still re-read all 1195 cards on every run | `reliability` + `catalogue` | **Largely resolved 2026-09-20 by deleting most of the gate instead of caching it.** Originally measured when the gate was change-scoped: four whole-catalogue scans (`check-egress.py` 1.43 s, `check-links.py` 1.16 s, `check-card-collisions.py` 0.94 s, `check-a11y.py` 0.59 s) were ~4.1 s of a 10.2 s card verify. The suite is now eight checks in ~3.6 s and three of those four moved behind `--deep`, so only `check-links.py` still re-reads every card on every run; the gate's slowest three are links 1.1 s, catalogue 0.9 s, counts 0.7 s. The per-file cache idea (keyed on the file's content hash *and the checker's own code* — the fingerprint pattern the deleted site-brain builder used) still stands if the gate ever grows slow again, and so does the reason it was never shipped: a stale cache is a gate that cannot fail, so it needs a test that corrupts the cache and proves the check still fails. Acceptance if revisited: per-check timings down in the run's own timing table, a cache-corruption test in `scripts/tests/`, `npm run verify:deep` green. | Unclaimed |
