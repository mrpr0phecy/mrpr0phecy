...
| P3-T6 | De-duplicate the homepage's catalogue payload | `reliability` + `catalogue` | Measured 2026-09-19: one homepage visit downloads three catalogue files — `cards/cards-lite.json` (31.5 KB gzip), `cards/cards.json` (144 KB) and `tools-index.json` (188 KB). The last is ~all title/description/category the full tier already carries; only `tags`, `popularity`, `featured` and the category icons are unique to it. Shipping a generated `cards-signals.json` for those fields and letting `discovery-app.js` reuse the tier already in flight would remove ~170 KB gzip per first visit. Not done here because it needs a new generator output, a `--check` gate and a browser-measured before/after (P0-M2), and a half-migrated discovery layer is worse than the duplication. Interim: `tools-index.json` is now fetched at `priority: 'low'` so it cannot outrank the first screen's card fragments. Acceptance: measured first-visit bytes down, `verify.sh` green, no change to what the browse chrome renders. | Unclaimed |
| P3-T7 | Placeholder and rating markup still interpolates cards.json strings | `privacy` + `reliability` | `createPlaceholder()`'s header and `updateCardRatingDisplay()`'s footer build HTML with `innerHTML` from `title`/`category` out of `cards.json` — the same class CONSTRAINTS.md hard line 4 forbids, and the class the search status messages were just fixed for. Left alone deliberately: both run per card (1194×) on the loader's measured hot path, and no browser is available here to prove a DOM-API rewrite costs nothing. Data is first-party and reviewed, so this is defence-in-depth, not a live vector. Acceptance: rewritten with `textContent` for the two dynamic strings, with a before/after placeholder-build timing recorded. | Unclaimed |

| P3-T8 | The card scanners still re-read all 1195 cards on every run | `reliability` + `catalogue` | Measured 2026-09-20, after the gate became change-scoped: a card edit runs 16 sections in ~10.2 s, and four of them are whole-catalogue scans that re-parse files nobody touched — `check-egress.py` 1.43 s, `check-links.py` 1.16 s, `check-card-collisions.py` 0.94 s, `check-a11y.py` 0.59 s, ~4.1 s of the 10.2 s. Each produces a per-file verdict, so a cache keyed on the file's content hash *and the checker's own code* (the fingerprint pattern `build-site-brain.py --check` already uses, and the argument `evaluate-site-brain.py` documents for its memoisation) would reduce a one-card verify to the changed card plus the cross-card comparison. Not shipped with the scoping work: a stale cache is a gate that cannot fail, which this repo treats as worse than a slow gate, and the invalidation key has to cover the checker's rules as well as the card's bytes — worth doing deliberately, with a test that corrupts the cache and proves the section still fails. Acceptance: per-section timings down in the run's own timing table, a cache-corruption test in `scripts/tests/`, `bash scripts/verify.sh --all` green. | Unclaimed |
