...
profitable, famous) — and [EXCELLENCE.md](EXCELLENCE.md) — the measurable
standard every item below serves — and [RESEARCH.md](RESEARCH.md) for why
this system was rebuilt. Standing rules: [DECISIONS.md](DECISIONS.md),
...
| ID | Outcome | Accountable profile | Evidence / acceptance | State |
|---|---|---|---|---|
| P1-R1 | Titles + descriptions that win the click — and the stay | `seo` | NavBoost (confirmed: DOJ testimony + 2024 API leak) ranks on click classifications over a ~13-month window: goodClicks (stay, don't return), badClicks (pogo-stick back to the SERP — a demotion signal), lastLongestClicks (the terminal click — strongest positive). Doctrine: be the result users settle on. CTR pass over indexable hubs: query + honest differentiator ("Free · No sign-up · Runs in browser"), descriptions promising the task outcome; then satisfy intent above the fold so the click sticks. Fixes the known weak spot first: index meta description ("Collection of essential tools…") undersells the page it sits on. Acceptance: unique, accurate, no invented reach; CTR above SERP average for the position, engagement rate > 60% where measurable; before/after in Search Console after 30 days. | Unclaimed · needs P0-M1 for priorities, copy fixes can start |
| P1-R2 | Prerendered static pages for the top 10–25 proven tools | `catalogue` + `seo` + `reliability` | The structural gap: all tools share client-side `tool.html?card=<slug>` — thin unique text, query-param URLs, invisible to non-JS crawlers. Build-time generator (same pattern as `build-home-prerender.py`/`build-sitemap.py`) emitting static per-tool pages with unique content per URL: how-to, worked example, FAQ, sources; card fragment stays the single implementation; server-side canonical + WebApplication/BreadcrumbList/FAQ schema. Start with the Search Console top 10–25 only — never all 1149 at once (scaled-content risk; March 2026 enforcement applies a sitewide weakest-link demotion, so one thin batch can drag the whole domain). Acceptance: Rich Results Test valid, `--check` drift gate in `verify.sh`, indexed within 30 days. | Pilot shipped on `arena/01a0a58d` (mortgage/BMI/compound + `tool-pages.test.js`, unmerged) · needs P0-M1 for the rollout list |
...
| P1-R8 | Entity authority: be a node, not a keyword | `seo` + owner | Branded web mentions correlate 0.664 with AI Overview citations vs 0.218 for backlinks — entity signals now outrank link signals for AI visibility. Ship: Organization `@id` + sameAs on index/about, Person `@id` + sameAs ladder (LinkedIn, GitHub, YouTube, trade profiles — identical name/photo/role), connected `@graph` (Organization↔WebSite↔Person), entity-defining copy (one-sentence definitions reused everywhere). Owner-side: Wikidata item (needs real identity + independent coverage), Crunchbase/LinkedIn company profiles, NAP consistency audit. Acceptance: Knowledge Graph Search API resolves the brand; LLM resolution test returns consistent bios; sameAs coherence 100%. | Unclaimed · staff schema now, owner identity items need O-10 |
| P1-R9 | Field-data performance watch (no new tracking) | `reliability` | Lab scores don't rank you — CrUX field data at p75 does, on a 28-day rolling window. Run scheduled PageSpeed Insights / CrUX API checks (off-site measurement, zero new on-page tracking) on index, tool shell, one guide, listen; alert on regression. Competitive target: INP ≤ 150ms (Google's bar is 200ms; top sites clear 150). Acceptance: monthly field-data table on BOARD.md; any red metric gets an OPEN.md item within a week. | Unclaimed · no owner block |
...
| P1-M4 | Sync-licensing page that closes | `music` + `finance` | Dedicated page: one-stop rights, moods/uses, clear pricing path ("£50–£500 YouTube/indie, £500–£5,000+ ads/games — ask for the one-page agreement"), enquiry route, honest catalogue facts. Take advice before signing the first meaningful licence (royalty tax treatment differs — FINANCE.md §2). Acceptance: page live, enquiry route tested, first 5 supervisor/agency emails drafted for the owner. | Built on `arena/01a0a58d` (`sync.html` + listen footer link, unmerged) · supervisor emails still to draft |
...
| P1-M6 | Layer the stack: newsletter, digital products, contained affiliate (proposals) | `growth` + owner | Display ads are one layer, never the strategy — and on tools they stay off (the licence buyer buys a clean embed). Staff prepare owner-ready proposals: (1) newsletter sponsorship ladder (niche CPMs run $50–100+; needs a provider choice — O-11); (2) digital products near-100% margin (e.g. UK tax-year toolkit, spreadsheet packs — scoped, priced, fulfilment-free); (3) affiliate rules that keep it contained (relevance test, disclosure, no review-farming). Nothing live without a signature. Acceptance: one proposal doc per line with costs, effort and first-step owner action. | Unclaimed · owner decisions |
...
| P2-D4 | Award-readiness path (jury standard, then submission) | `visual-design` + owner | Bar first, trophy second. Bring one surface (listen.html is the candidate: a contained, expressive music experience) to SOTD standard against the published criteria — Design 40 / Usability 30 / Creativity 20 / Content 10, mobile-native, CWV green, keyboard + reduced-motion complete — with the evidence checklist filled. Then the owner decides whether to submit (Awwwards ~$75, CSSDA ~$49). Win or lose, the process leaves a better page. Acceptance: checklist green with browser evidence; submission is an owner call (O-12), never staff-spent money. | Unclaimed · prep is staff, submission is owner |
...
| P2-P2 | Outreach engine: packs + journalist sourcing + data studies | `seo` + `growth` + `music` | Three layers, all offering genuine value: (1) resource-page pack (.ac.uk/.org.uk/libraries: free, no signup, runs in browser, accessible) + vertical page draft ("free mortgage calculator for your website" → licence funnel); (2) journalist sourcing on Connectively + Featured + Qwoted (or UK: ResponseSource) — first-hour responses win 60%+ more placements; owner sends, staff draft the quotable lines; (3) digital-PR data studies on a cadence (finance-bug findings, catalogue stats, calculator-methodology pieces) — consistent data PR earns 3–5× the high-authority links of outreach alone. Cold outreach converts 1–3%, warm 15–30%: relationships first. Acceptance: packs ready; sourcing accounts live; one study per quarter minimum; links earned tracked monthly. | Unclaimed |
...
| P2-P4 | Owned audience: newsletter pilot (owner decision) | `growth` + owner | 60% of searches end without a click — rented land. A newsletter is the direct relationship no algorithm can take: subscribers outperform larger anonymous audiences, sponsor CPMs run $50–100+ in niches, and the list is first-party data that lifts on-site ad rates later. Owner picks the provider (Buttondown/Substack/etc. — account + cost + privacy review); staff prepare the signup surface spec (honest value prop, no dark patterns, double opt-in) + first 4 issues outline from existing catalogue depth. Acceptance: provider live, signup converting, 4 issues shipped. | Unclaimed · owner decision (O-11) |
...
| O-10 | Entity identity: Wikidata item + company profiles (P1-R8) | Seeds Knowledge Graph recognition (3–12 month payoff for AI citations) | Identity + a little profile admin |
...
| O-12 | Award submission yes/no once P2-D4 prep is green | Trophy optional; the prep work improves the page regardless | ~$75 + one ruling |
