...
## D-010 — A rule that cannot fail the build is not a rule

**Decided:** 2026-09-04 · **Owner:** owner mandate ("rewrite the rules for the
good of the site") · **Status:** binding

Every recurring failure in this repo's history has the same shape: a correct
rule, written in prose, that humans and agents were expected to remember.

| Rule, as written | Outcome |
|---|---|
| "Bump the count across 8 files" (ARCHITECTURE.md §9, stated 6×) | Shipped 250/483/500/562/602/612/622/632/634 concurrently — including on donate and sponsor, the money pages |
| "Cards make zero network calls" (law since day one) | 27 cards calling out; `wifi-qr-generator` posted Wi-Fi passwords to a third party under the words *100% private* |
| "Regenerate the sitemap with this snippet" (§6) | The documented snippet had no `noindex` filter and re-added 3 excluded pages every run |
| "Unique IDs across all cards" | Enforced only by a scanner that reported 5 template-literal false positives and 0 real ones |

The failure is never discipline. It is that **prose does not execute**.

**The rule:** when a constraint matters, it ships as a check in `scripts/`
wired into `scripts/verify.sh` and CI — in the same change that states it.
A constraint that exists only as a sentence is a suggestion, and is expected
to decay. When adding a rule to any document, the reviewer's first question
is *"what fails if this is violated?"* If the answer is "nothing", the rule
is not finished.

**Derived corollaries** (full text in AGENTS.md, top section):

1. **Derive, don't duplicate.** The tool count comes from `ls cards/`; the
   sitemap comes from `git ls-files`. Both are now generated by
   `scripts/sync-counts.py` and `scripts/build-sitemap.py`. Hand-editing
   either is a never-do.
2. **The check owns the rule.** Where a guardrail and a document disagree,
   the guardrail wins and the document is the bug.
3. **Narrow the check until it is silent.** Zero-warning is the only tolerable
   resting state; a noisy scanner protects nothing. (`scan-seo.py` hid 4 real
   defects behind 26 spurious ones until it was taught about `noindex`.)
4. **Never make history lie.** Generators rewrite claims, never the changelog.
   `sync-counts.py` freezes everything from ARCHITECTURE.md §9 onward.

**Also settled here:** fixing a broken *check* is as legitimate and as urgent
as fixing broken *code*, and needs no owner sign-off. Routing around a red
check, or loosening one to green without understanding it, is a never-do.

**Verify is now 11 checks** (was 7 on 2026-09-03): + accessibility, + egress,
+ count claims, + sitemap freshness. All four were previously "rules".
