...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 640 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 640 tools
│   └── <tool-name>.html    640 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 640 cards.** They share one
...
tree, so a sparse checkout does not silently drop the 640 cards:
...
**ID collisions across cards.** All 640 share one DOM. See §3.
...
# Music work (skip images and the 640 cards):
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Since all 640 cards share one DOM, `getElementById` can bind to the
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  640 offline-first tools it is a large caching win (network-first for HTML,
