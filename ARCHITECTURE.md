...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 634 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 634 tools
│   └── <tool-name>.html    634 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 634 cards.** They share one
...
tree, so a sparse checkout does not silently drop the 634 cards:
...
**ID collisions across cards.** All 634 share one DOM. See §3.
...
# Music work (skip images and the 634 cards):
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Since all 634 cards share one DOM, `getElementById` can bind to the
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  634 offline-first tools it is a large caching win (network-first for HTML,
