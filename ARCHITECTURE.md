...
# 4. Refresh the AI/plain surface (llms.txt, llms-full.txt, tools-index.html)
node scripts/generate-ai-index.js

# 5. Bump the count in index.html: "Search 500+ free tools" -> 501+

# 6. Commit, push, wait ~50s, then verify live:
...
### AI-facing surface (agents, crawlers, zero-JS users)

The catalogue is deliberately machine-readable. Four generated/handwritten
artefacts live at the root and must stay in sync with `cards/cards.json`:

| File | What it is |
|---|---|
| `llms.txt` | concise `/llms.txt` index for AI agents (endpoints + category map) |
| `llms-full.txt` | every tool, one line each, grouped by category |
| `tools-index.html` | static zero-JS directory of all tools (crawlable, screen-reader friendly) |
| `ai.html` | handwritten guide: URL patterns, embed snippet, manifest usage |

All but `ai.html` are generated by `node scripts/generate-ai-index.js` — run it
after any catalogue change (it is step 4 of "Adding a tool" above).
`robots.txt` points agents at `llms.txt`; the homepage footer links all four.
The homepage itself also accepts deep links: `?q=` (search), `?view=list|cards`,
`?category=`, and `?expand=<slug>` (opens that tool expanded inline in the
default list view). `tool.html` accepts `&embed=1` to strip page chrome and
report its height via `postMessage` (`tmusitw:height`) for iframe embedding.
...
prio  = {"listen.html":("1.0","weekly"), "music.html":("0.9","weekly"),
         ...
         "tools-index.html":("0.8","weekly"), "ai.html":("0.6","monthly")}
