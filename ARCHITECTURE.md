...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 1295 self-contained browser tools | `index.html` | People searching for a specific tool |
...
│   ├── cards.json          Generated full index of all 1295 tools (descriptions feed search)
│   └── <tool-name>.html    1295 tool fragments (NOT full documents)
...
- **One place a tool can go wrong.** `tool.html` fails the same way for all
  1,295 tools, and `scripts/check-tool-graph.py` proves every link into it
...
- **`[data-explore="json"]`** — an empty container. Everything below the fold
  (the filterable list of all 1,295 tools) is fetched on scroll and built 60
...
2. **Element IDs must be globally unique across all 1295 cards.** They share one
...
| 74 | SaaS & Business Killers | | 19 | AI & Autonomous Agents |
| 72 | Writing & Language | | 17 | Mind-Blowing Demos |
| 70 | Algorithms & Computer Science | | 13 | Lucid Dreaming & Sleep |
| 64 | Mathematics | | 11 | Survival & Emergency Readiness |
| 56 | Sports | | 10 | Anime & Otaku Culture |
| 52 | Interactive Art & Living Worlds | | 10 | Aquatics & Fishkeeping |
...
Total: 1295 tools in 29 categories.
---
...
`sitemap.xml` lists all 1197 indexable pages (including 1295 cards). Build it
...
# Music work (skip images and the 1295 cards):
...
1295 tools in `cards/` across 29 categories, one shared DOM, every derived
