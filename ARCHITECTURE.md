...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 1230 self-contained browser tools | `index.html` | People searching for a specific tool |
...
│   ├── cards.json          Generated full index of all 1230 tools (descriptions feed search)
│   └── <tool-name>.html    1230 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 1230 cards.** They share one
...
Total: 1230 tools in 28 categories.
---
...
`sitemap.xml` lists all 1197 indexable pages (including 1230 cards). Build it
...
**ID collisions across cards.** All 1230 share one DOM. See §3. A parked subtree
...
# Music work (skip images and the 1230 cards):
...
1230 tools in `cards/` across 28 categories, one shared DOM, every derived
