...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 1312 self-contained browser tools | `index.html` | People searching for a specific tool |
...
│   └── <tool-name>.html    1312 tool fragments (NOT full documents)
...
- **One place a tool can go wrong.** `tool.html` fails the same way for all
  1,312 tools, and `scripts/check-tool-graph.py` proves every link into it
...
- **`[data-explore="json"]`** — an empty container. Everything below the fold
  (the filterable list of all 1,312 tools) is fetched on scroll and built 60
...
2. **Element IDs must be globally unique across all 1312 cards.** They share one
...
`sitemap.xml` lists all 1197 indexable pages (including 1312 cards). Build it
