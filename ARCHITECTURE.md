...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 647 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 647 tools
│   └── <tool-name>.html    647 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 647 cards.** They share one
...
| 48 | Writing & Language | | 10 | Anime & Otaku Culture |
...
`sitemap.xml` lists all 687 pages. Build it from git rather than the working
tree, so a sparse checkout does not silently drop the 647 cards:
...
**ID collisions across cards.** All 647 share one DOM. See §3.
...
# Music work (skip images and the 647 cards):
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Since all 647 cards share one DOM, `getElementById` can bind to the
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  647 offline-first tools it is a large caching win (network-first for HTML,
