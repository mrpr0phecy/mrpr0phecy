...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 632 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 632 tools
│   └── <tool-name>.html    632 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 632 cards.** They share one
...
tree, so a sparse checkout does not silently drop the 632 cards:
...
**ID collisions across cards.** All 632 share one DOM. See §3.
...
# Music work (skip images and the 632 cards):
...
Added to `homeDIYList` in `generate-cards-json.js`; tool count is now **632**
...
now **632** (updated across README, ARCHITECTURE, INCOME, AGENTS, AGENT_ACCESS,
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Since all 632 cards share one DOM, `getElementById` can bind to the
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  632 offline-first tools it is a large caching win (network-first for HTML,
