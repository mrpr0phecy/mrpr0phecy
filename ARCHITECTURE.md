...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 562 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 562 tools
...
├── sitemap.xml             All 602 URLs, generated (§6)
...
2. **Element IDs must be globally unique across all 562 cards.** They share one
...
# 4. Bump "Search <N> free tools" in index.html to the new card total.
#    scripts/check-cards.py fails if it disagrees with cards.json.
...
| 109 | Productivity & Lifestyle | | 10 | AI & Autonomous Agents |
| 47 | Writing & Language | | 10 | Anime & Otaku Culture |
| 35 | Finance & Money | | 10 | Aquatics & Fishkeeping |
| 28 | Mathematics | | 10 | Astronomy & Space |
| 23 | Health & Fitness | | 10 | Birdwatching & Ornithology |
| 23 | Music & Audio | | 10 | Dogs & Canine Care |
| 17 | Home & DIY | | 10 | MrProphecy Arcade |
| 15 | Culinary & Food Science | | 10 | Natural Remedies & Herbs |
| 12 | Museum & Collection | | 10 | Wellbeing & Community |
| 11 | Interactive Art & Living Worlds | | 7 | Virtual Worlds & Gaming |
| 11 | Lucid Dreaming & Sleep | |  |  |

*23 categories, 562 tools — derived from `cards/cards.json`, the generated source of truth. `scripts/check-cards.py` fails the build if this heading's count drifts from the catalogue.*
...
**ID collisions across cards.** All 562 share one DOM. See §3.
...
# Music work (skip images and the 562 cards):
...
**Systems maintenance pass (2026-09-02)** — the guardrails themselves were the
problem this time. Nothing on the live site changed except one broken link.

- **`check-cards.py` was reporting five duplicate ids that do not exist.** It
  scanned `id="${item.id}"` inside `<script>` blocks as literal ids, so every
  generator loop in every card looked like a collision (54 such sites
  site-wide). Real ids generated by JS *are* ids at runtime — cards share one
  DOM — so stripping scripts would have hidden genuine bugs. The check now skips
  only `${…}` interpolation. Verified: **0 duplicate ids** across all 562 cards.
- **The tool count had drifted in four documents.** `cards.json`, `index.html`
  and ARCHITECTURE.md §1 all said **562**; README, AGENTS.md, AGENT_ACCESS.md
  and INCOME.md all still said **500**. AGENTS.md even instructed agents to
  rewrite `Search 500` → `Search 501`, which would have shipped a wrong count.
  All corrected, and the instructions now say "set it to the new card total".
- **Counts and categories are now enforced, not trusted.** `check-cards.py`
  fails the build if a prose doc's stated count, `index.html`'s JSON-LD
  `numberOfItems`, its category pills, or this file's category table disagree
  with `cards/cards.json`. The hardcoded `KNOWN_CATEGORIES` set in the script was
  deleted — it had drifted to include a category the generator cannot emit.
- **The category table above was wrong.** It summed to **483** under a heading
  claiming 562, listed `Boxing & Fight Scoring` (10 tools) which does not exist
  in `cards.json`, and omitted five real categories. Regenerated from
  `cards/cards.json`; it now sums to exactly 562 across 23 categories.
- **One live 404 fixed.** `cards/mrprophecy-tour-manager.html` linked
  `href="listen.html"` — `/cards/listen.html` when the card is opened directly.
  The identical bug is documented as fixed below; this instance was missed.
  Now `../listen.html`, and `check-cards.py` fails the build if any card does it
  again.
- **Stale open item retired.** The whole-file id collisions listed as open below
  (`leanbodymass`↔`lease` and friends) were already fixed; they stayed on the
  list because the check that watched them only ever emitted false positives.
- **`.github/workflows/ai-developer.yml` deleted** (owner decision, 2026-09-02).
  It ran `node scripts/ai-developer.js`, a file that never existed in any commit
  on any branch, on a `0 6 * * 1,4` schedule — so it failed twice a week, and the
  2026-09-01 manual run failed in 15 s at exactly that step. It also held
  `contents: write` and `pull-requests: write` for no benefit. Its purpose —
  agents adding and improving tools — is now done directly by agent sessions,
  coordinated on `staff/BOARD.md`. `agent-guardrails.yml` is the only workflow
  left; if a future automation is added, it must run `verify.sh` and open a PR
  rather than push to `main`.

**Still open, needs an owner decision** (found this pass):

- **Ten boxing tools are filed under the wrong category.** `boxing-*.html` (10
  cards) are categorised `Productivity & Lifestyle` because
  `generate-cards-json.js` has no `boxingList` router, yet both this document and
  the old `KNOWN_CATEGORIES` set promised a `Boxing & Fight Scoring` category.
  The script overwrites categories on every run, so the category can never
  appear. Adding it means a router entry plus a filter pill and JSON-LD item in
  `index.html` — a visible catalogue change, so it needs a decision.
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Re-verified 2026-09-02: still exactly 17.

  *Previously listed here as open:* whole-file **id collisions** between copied
  cards (`leanbodymass`↔`lease`, `moving`↔`music-theory`, `salary`↔`salarycompare`,
  and others). **These are fixed** — re-checked 2026-09-02, the catalogue now has
  **0 duplicate ids**. The item survived this long because the guardrail that was
  supposed to watch it was reporting five false positives instead: it scanned
  `id="${item.id}"` template *placeholders* inside `<script>` blocks as if they
  were literal ids. `check-cards.py` now skips `${…}` interpolation (54 sites
  site-wide) while still scanning JS-generated ids, which are real ids at
  runtime because every card shares one DOM.
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  562 offline-first tools it is a large caching win (network-first for HTML,
