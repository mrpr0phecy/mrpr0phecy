...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 729 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 729 tools
│   └── <tool-name>.html    729 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 729 cards.** They share one
...
`sitemap.xml` lists all 729 pages. Build it from git rather than the working
...
**654** and the sitemap has **729** URLs (updated across README, ARCHITECTURE,
...
At that point, the tool count reached **664** across **27 categories**, and the sitemap had **729**
...
**27 categories**; `sitemap.xml` had **729 URLs**. Counts, the homepage ItemList,
...
across **27 categories**; `sitemap.xml` has **729 URLs**. Counts, homepage
