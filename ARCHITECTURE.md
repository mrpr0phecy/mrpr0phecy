...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 809 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── categories/             27 category landings + index (real URLs, not hashes)
├── calculators.html        Intent hub — calculator-shaped tools
├── converters.html         Intent hub — converters
├── generators.html         Intent hub — generators
├── developer-tools.html    Intent hub — developer utilities
├── about-tools.html        E-E-A-T / who-makes-this page (Luton)
├── feed.xml                RSS of every tool URL
├── opensearch.xml          Browser search plugin → all-tools.html?q=
...
├── .well-known/llms.txt    Same catalogue, well-known location
├── tools/
...
├── scripts/generate-discoverability.js  tools/ + hubs + llms.txt + sitemap
├── scripts/seo-hubs.js                  category / intent / about / RSS writer
│
...
| URL | Role |
|---|---|
| `/tools/<slug>.html` | Canonical page for one tool — unique title, description, OG, FAQ + WebApplication JSON-LD |
| `/all-tools.html` | Crawlable A–Z directory; every tool is a real `<a href>` |
| `/categories/<slug>.html` | One landing page per category (27). Index: `/categories/` |
| `/calculators.html` `/converters.html` `/generators.html` `/developer-tools.html` | Intent hubs |
| `/about-tools.html` | Who builds the tools (Luton), how they run, contact |
| `/llms.txt` | Short catalogue for AI agents (llmstxt.org). Full list: `/llms-full.txt`. Mirror: `/.well-known/llms.txt` |
| `/feed.xml` | RSS of every canonical tool URL |
| `/opensearch.xml` | OpenSearch → `all-tools.html?q={searchTerms}` |
...
### Categories (809 tools)

| Count | Category | | Count | Category |
|---|---|---|---|---|
| 122 | Science & Engineering | | 51 | SaaS & Business Killers |
| 123 | Productivity & Lifestyle | | 11 | Lucid Dreaming & Sleep |
| 49 | Writing & Language | | 10 | Wellbeing & Community |
| 38 | Finance & Money | | 10 | Natural Remedies & Herbs |
| 53 | Sports | | 10 | AI & Autonomous Agents |
| 30 | Mathematics | | 11 | Astronomy & Space |
| 23 | Music & Audio | | 10 | Anime & Otaku Culture |
| 26 | Health & Fitness | | 10 | Aquatics & Fishkeeping |
...
That writes `tools/<slug>.html`, `all-tools.html`, category landings,
intent hubs, `about-tools.html`, `feed.xml`, `humans.txt`, `llms.txt` /
`llms-full.txt` / `ai.txt` / `.well-known/llms.txt`, and rebuilds
`sitemap.xml` from `git ls-files` (static pages) plus every tool in
`cards.json` plus the generated hubs. Excluded: `404.html`, `hokidea.html`,
`indexbeta.html`, `tool.html`. Homepage SearchAction is an EntryPoint
template at `all-tools.html?q={search_term_string}`. Category links on the
homepage ItemList and footer go to `/categories/<slug>.html`, not hash URLs.
Tool wrappers use the system font stack (no Google Fonts) and do not carry
Product B music chrome.
...
**Added 2026-09-07** — Product A **second SEO pass** (more crawlable landings).
`categories/<slug>.html` (27) plus `categories/index.html`; intent hubs
`calculators.html`, `converters.html`, `generators.html`,
`developer-tools.html`; `about-tools.html` (Luton / E-E-A-T);
`opensearch.xml`, `feed.xml`, `humans.txt`, `.well-known/llms.txt`. Tool
pages dropped Google Fonts (system stack), gained FAQ JSON-LD, prev/next in
category, and breadcrumbs that point at category URLs not hashes. Homepage
publisher Organization is the tools site (not `listen.html`); SearchAction
uses an EntryPoint `urlTemplate`; ItemList + footer-dir link
`/categories/<slug>.html`. Donate uses `og-tools.png` and `lang=en-GB`.
Still regenerated by `node scripts/generate-discoverability.js` — do **not**
re-run `generate-cards-json.js` just to refresh this surface (it overwrites
categories).
...
**654** and the sitemap has **809** URLs (updated across README, ARCHITECTURE,
...
At that point, the tool count reached **664** across **27 categories**, and the sitemap had **809**
...
**27 categories**; `sitemap.xml` had **809 URLs**. Counts, the homepage ItemList,
...
across **27 categories**; `sitemap.xml` has **809 URLs**. Counts, homepage
