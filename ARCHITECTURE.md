...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 1313 self-contained browser tools | `index.html` | People searching for a specific tool |
...
│   ├── cards.json          Generated full index of all 1313 tools (descriptions feed search)
│   └── <tool-name>.html    1313 tool fragments (NOT full documents)
...
- **One place a tool can go wrong.** `tool.html` fails the same way for all
  1,313 tools, and `scripts/check-tool-graph.py` proves every link into it
...
- **`[data-explore="json"]`** — an empty container. Everything below the fold
  (the filterable list of all 1,313 tools) is fetched on scroll and built 60
...
2. **Element IDs must be globally unique across all 1313 cards.** They share one
...
bash scripts/verify.sh --deep   # shared infrastructure changes; see AGENTS.md §1
...
### Categories (1313 tools)

Derived from `cards/cards.json` — regenerate rather than hand-edit.

| Count | Category | | Count | Category |
|---|---|---|---|---|
| 205 | Science & Engineering | | 29 | MrProphecy Arcade |
| 172 | Productivity & Lifestyle | | 29 | Museum & Collection |
| 94 | Finance & Money | | 22 | Virtual Worlds & Gaming |
| 79 | SaaS & Business Killers | | 19 | AI & Autonomous Agents |
| 74 | Algorithms & Computer Science | | 17 | Mind-Blowing Demos |
| 73 | Writing & Language | | 13 | Lucid Dreaming & Sleep |
| 65 | Mathematics | | 11 | Survival & Emergency Readiness |
| 56 | Sports | | 10 | Anime & Otaku Culture |
| 54 | Interactive Art & Living Worlds | | 10 | Aquatics & Fishkeeping |
| 48 | Health & Fitness | | 10 | Birdwatching & Ornithology |
| 44 | Home & DIY | | 10 | Dogs & Canine Care |
...
Total: 1313 tools in 29 categories.
---
...
`sitemap.xml` lists all 1197 indexable pages (including 1313 cards). Build it
...
# Music work (skip images and the 1313 cards):
...
1313 tools in `cards/` across 29 categories, one shared DOM, every derived
