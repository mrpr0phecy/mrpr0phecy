...
│
├── listen.html             Product B: music hub — the main music page
├── music.html              Older, longer music page (bio + links)
├── youtubepromo{,1,2,3}.html   Four overlapping promo/EPK pages
...
- Four overlapping promo pages (`youtubepromo.html`, `1`, `2`, `3`) say much the
  same thing. `youtubepromo.html` is orphaned — nothing links to it. Consolidating
  into one strong EPK page would concentrate ranking signals instead of splitting
  them four ways.
- `music.html` and `listen.html` overlap. `listen.html` is the stronger page;
  consider making `music.html` redirect to it, or narrowing it to a long-form bio.
