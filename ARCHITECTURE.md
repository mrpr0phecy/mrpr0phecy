...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 774 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 774 tools
│   └── <tool-name>.html    774 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 774 cards.** They share one
...
`sitemap.xml` lists all 774 pages. Build it from git rather than the working
...
**654** and the sitemap has **774** URLs (updated across README, ARCHITECTURE,
...
At that point, the tool count reached **664** across **27 categories**, and the sitemap had **774**
...
**27 categories**; `sitemap.xml` had **774 URLs**. Counts, the homepage ItemList,
...
across **27 categories**; `sitemap.xml` has **774 URLs**. Counts, homepage
