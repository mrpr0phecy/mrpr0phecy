...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 784 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 784 tools
│   └── <tool-name>.html    784 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 784 cards.** They share one
...
`sitemap.xml` lists all 784 pages. Build it from git rather than the working
...
**654** and the sitemap has **784** URLs (updated across README, ARCHITECTURE,
...
At that point, the tool count reached **664** across **27 categories**, and the sitemap had **784**
...
**27 categories**; `sitemap.xml` had **784 URLs**. Counts, the homepage ItemList,
...
across **27 categories**; `sitemap.xml` has **784 URLs**. Counts, homepage
