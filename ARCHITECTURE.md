...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 759 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 759 tools
│   └── <tool-name>.html    759 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 759 cards.** They share one
...
`sitemap.xml` lists all 759 pages. Build it from git rather than the working
...
**654** and the sitemap has **759** URLs (updated across README, ARCHITECTURE,
...
At that point, the tool count reached **664** across **27 categories**, and the sitemap had **759**
...
**27 categories**; `sitemap.xml` had **759 URLs**. Counts, the homepage ItemList,
...
across **27 categories**; `sitemap.xml` has **759 URLs**. Counts, homepage
