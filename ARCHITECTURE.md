...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 1240 self-contained browser tools | `index.html` | People searching for a specific tool |
...
│   ├── cards.json          Generated full index of all 1240 tools (descriptions feed search)
│   └── <tool-name>.html    1240 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 1240 cards.** They share one
...
Total: 1240 tools in 28 categories.
---
...
`sitemap.xml` lists all 1197 indexable pages (including 1240 cards). Build it
...
**ID collisions across cards.** All 1240 share one DOM. See §3. A parked subtree
...
# Music work (skip images and the 1240 cards):
...
1240 tools in `cards/` across 28 categories, one shared DOM, every derived
