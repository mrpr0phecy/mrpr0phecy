...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 645 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 645 tools
│   └── <tool-name>.html    645 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 645 cards.** They share one
...
`sitemap.xml` lists all 685 pages. Build it from git rather than the working
tree, so a sparse checkout does not silently drop the 645 cards:
...
**ID collisions across cards.** All 645 share one DOM. See §3.
...
# Music work (skip images and the 645 cards):
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Since all 645 cards share one DOM, `getElementById` can bind to the
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  645 offline-first tools it is a large caching win (network-first for HTML,
