...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 662 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 662 tools
│   └── <tool-name>.html    662 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 662 cards.** They share one
...
`sitemap.xml` lists all 702 pages. Build it from git rather than the working
tree, so a sparse checkout does not silently drop the 662 cards:
...
**ID collisions across cards.** All 662 share one DOM. See §3.
...
# Music work (skip images and the 662 cards):
...
a **coat-or-no-coat weather** advisor. Tool count is now **662** (updated across
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Since all 662 cards share one DOM, `getElementById` can bind to the
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  662 offline-first tools it is a large caching win (network-first for HTML,
