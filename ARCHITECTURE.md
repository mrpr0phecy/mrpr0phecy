...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 648 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 648 tools
│   └── <tool-name>.html    648 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 648 cards.** They share one
...
`sitemap.xml` lists all 688 pages. Build it from git rather than the working
tree, so a sparse checkout does not silently drop the 648 cards:
...
**ID collisions across cards.** All 648 share one DOM. See §3.
...
# Music work (skip images and the 648 cards):
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Since all 648 cards share one DOM, `getElementById` can bind to the
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  648 offline-first tools it is a large caching win (network-first for HTML,
