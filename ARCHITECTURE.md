...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 656 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 656 tools
│   └── <tool-name>.html    656 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 656 cards.** They share one
...
`sitemap.xml` lists all 696 pages. Build it from git rather than the working
tree, so a sparse checkout does not silently drop the 656 cards:
...
**ID collisions across cards.** All 656 share one DOM. See §3.
...
# Music work (skip images and the 656 cards):
...
a **coat-or-no-coat weather** advisor. Tool count is now **656** (updated across
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Since all 656 cards share one DOM, `getElementById` can bind to the
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  656 offline-first tools it is a large caching win (network-first for HTML,
