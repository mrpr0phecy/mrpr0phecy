...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 1306 self-contained browser tools | `index.html` | People searching for a specific tool |
...
│   ├── cards.json          Generated full index of all 1306 tools (descriptions feed search)
│   └── <tool-name>.html    1306 tool fragments (NOT full documents)
...
- **One place a tool can go wrong.** `tool.html` fails the same way for all
  1,306 tools, and `scripts/check-tool-graph.py` proves every link into it
...
- **`[data-explore="json"]`** — an empty container. Everything below the fold
  (the filterable list of all 1,306 tools) is fetched on scroll and built 60
...
2. **Element IDs must be globally unique across all 1306 cards.** They share one
...
Total: 1306 tools in 29 categories.
---
...
`sitemap.xml` lists all 1197 indexable pages (including 1306 cards). Build it
...
# Music work (skip images and the 1306 cards):
...
1306 tools in `cards/` across 29 categories, one shared DOM, every derived
