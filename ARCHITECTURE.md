...
|---|---|---|---|
| **A** | **The Most Useful Site In The World** — 661 self-contained browser tools | `index.html` | People searching for a specific tool |
...
├── cards/
│   ├── cards.json          Generated index of all 661 tools
│   └── <tool-name>.html    661 tool fragments (NOT full documents)
...
2. **Element IDs must be globally unique across all 661 cards.** They share one
...
`sitemap.xml` lists all 701 pages. Build it from git rather than the working
tree, so a sparse checkout does not silently drop the 661 cards:
...
**ID collisions across cards.** All 661 share one DOM. See §3.
...
# Music work (skip images and the 661 cards):
...
a **coat-or-no-coat weather** advisor. Tool count is now **661** (updated across
...
- **17 `<label for=...>` associations point at no element** (they label button
  ...
  adding `aria-labelledby`. Since all 661 cards share one DOM, `getElementById` can bind to the
...
- **`sw.js` is still unregistered** — see the open question below. For a site of
  661 offline-first tools it is a large caching win (network-first for HTML,
