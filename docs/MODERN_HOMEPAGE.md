...
- New `sw.js` (module + classic fallback):
  - `STATIC_CACHE`: precaches `/`, `cards.json`, `tools-index.html`, `risk-notices.js`
  ...
  - `WARM_CACHE` message from page to warm `cards.json`
...
- **fetchpriority**: `high` for `cards.json` preload and fast-path fetches, `low` for SW
- **modulepreload** for SW, **preload** for `cards.json`
...
## Files Changed
- `index.html`: +~800 lines of modern CSS + ~200 lines JS helpers, popover attributes, speculation rules, import map, manifest link
- `sw.js`: new, 6.8KB
