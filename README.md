...
**1,260 free tools that run entirely in your browser — no backend, no signup, no tracking, no ads.**
...
| **Stack** | Static HTML/CSS/JS — **no build step in production, no framework, no dependencies, no backend**. All 1,260 tools are fragments in `cards/` |
| **Catalogue** | **1,260 tools · 29 categories · 67 top-level pages** — everything derived from `cards/cards.json` via `npm run build`. Tool count is not a growth target |
