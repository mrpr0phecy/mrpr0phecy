...
#  13. AI discovery            — scripts/check-ai-discovery.py
set -u
cd "$(dirname "$0")/.." || exit 1
ROOT=$(pwd)
...
section "12/13 staff facility configuration and regression tests"
...
section "13/13 AI discovery surface (check-ai-discovery.py)"
if python3 scripts/check-ai-discovery.py; then ok "robots, llms.txt, markdown, schema, IndexNow, MCP all wired"; else fail "AI discovery regression — assistants are losing the site"; fi
