...
#   7. finance correctness    — scripts/check-finance.js (money tools)
#   8. git state              — uncommitted changes reported (not failed)
set -u
cd "$(dirname "$0")/.." || exit 1
ROOT=$(pwd)
...
section "7/8 finance correctness (check-finance.js)"
if command -v node >/dev/null 2>&1; then
  if node scripts/check-finance.js >/tmp/fin.log 2>&1; then
    ok "money tools match statutory rates"
  else
    fail "finance checks failed:"; sed -n '/FAIL/p' /tmp/fin.log | head -10
  fi
else
  note "node not available — skipped"; NOTES=$((NOTES+1))
fi

section "8/8 git state"
