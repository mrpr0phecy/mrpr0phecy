...
Forty-six cards shipped with JavaScript that compiled cleanly and still failed
...
when you clicked it:
  ...
  broken HTML in the handler attribute itself
    oscilloscope            the Radio preset read onclick="oscLoadPreset("radio")"
                            -- a double quote inside a double-quoted attribute,
                            so the handler was the fragment oscLoadPreset( and
                            clicking it threw a SyntaxError
    carol-karaoke           the Lights theme read onclick="ckSetTheme="lights")",
                            the same typo; its three sibling theme buttons were
                            written correctly
    linux-log-analyzer      escaped the single quotes in its quick commands but
                            not the double quotes they contain -- grep " 5xx " --
                            so every generated handler was truncated
    studyplanner            two of its six inline handlers: one escaped double
                            quotes as \" (which does nothing in HTML) and one
                            was missing its setInterval interval and a closing
                            brace

  A scan of all 3,254 onclick-family attributes in markup, each parsed as
  JavaScript, found exactly two of these. The other two are generated by the
  card's own script, so only running the card finds them.

None of those are syntax errors. A compile check passes every one.

Forty-six is the count of distinct cards on this branch, not a tally kept
by hand — recount with `git diff --name-only <base>..HEAD -- cards/ | wc -l`
before editing this list. It has drifted five times now.
